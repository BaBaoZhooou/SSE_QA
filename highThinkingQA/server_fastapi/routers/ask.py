...
from server_fastapi.auth.deps import AuthContext, require_ask_auth_context
...
@router.post("/api/ask")
async def ask_v1(request: Request, context: AuthContext = Depends(require_ask_auth_context)):
...
async def ask_v1_mode(
    mode: str,
    request: Request,
    context: AuthContext = Depends(require_ask_auth_context),
):
...
async def ask_stream_v1(request: Request, context: AuthContext = Depends(require_ask_auth_context)):
    ask_request = _bind_auth_context(await _parse_request_or_raise(request), context)
    trace_id = get_trace_id()
