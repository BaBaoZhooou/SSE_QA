...
def require_ask_auth_context(token: str | None = Depends(get_bearer_token)) -> AuthContext:
    if token:
        return require_auth_context(token)
    if _anonymous_ask_enabled():
        return AuthContext(user_id=0, role="anonymous", username="anonymous")
    return require_auth_context(token)


def _anonymous_ask_enabled() -> bool:
    raw = str(
        os.getenv("HIGHTHINKINGQA_ANONYMOUS_ASK_ENABLED")
        or os.getenv("QA_ANONYMOUS_ASK_ENABLED")
        or ""
    ).strip().lower()
