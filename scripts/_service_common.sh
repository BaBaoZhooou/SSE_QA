...
# Optional unified YAML config; layered .env still loads afterward and can override.
SYSTEM_YAML="${SYSTEM_YAML:-$CONFIG_DIR/system.yaml}"
if [[ -f "$SYSTEM_YAML" ]]; then
  if python3 "$ROOT_DIR/scripts/render_system_config.py" --export-shell "$SYSTEM_YAML" >/tmp/qa_system_yaml_exports.$$ 2>/dev/null; then
    # shellcheck disable=SC1090
    source /tmp/qa_system_yaml_exports.$$
    rm -f /tmp/qa_system_yaml_exports.$$
  else
    echo "warning: failed to load $SYSTEM_YAML (install PyYAML: pip install pyyaml); using .env only" >&2
  fi
fi

# REDIS_KEY_PREFIX / PROMPTS_DIR differ per service; global export overrides service config.
unset REDIS_KEY_PREFIX
unset PROMPTS_DIR

MONOREPO_CONFIG_ENV_FILES="$CONFIG_DIR/config.shared.env:$CONFIG_DIR/config.secret.env"
source "$ROOT_DIR/scripts/env_file_loader.sh"
capture_env_file_loader_process_keys

if [[ -n "${QA_SERVICES:-}" ]]; then
  # shellcheck disable=SC2206
  SERVICES=(${QA_SERVICES})
else
  SERVICES=(fastQA highThinkingQA patent gateway)
fi
...
gateway_env_files() {
  ...
  echo "${GATEWAY_ENV_FILES:-$MONOREPO_CONFIG_ENV_FILES:$ROOT_DIR/gateway/config.env:$ROOT_DIR/gateway/config.shared.env:$ROOT_DIR/gateway/config.secret.env:$ROOT_DIR/gateway/.env:$shared_env_files:$config_dir_default/config.shared.env:$config_dir_default/config.secret.env:$config_dir_default/.env:$config_dir_default/config.env}"
}
...
# CLI: bash scripts/_service_common.sh gateway:start
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  if [[ $# -ne 1 ]]; then
    echo "usage: $0 <service>:<start|stop|status>" >&2
    exit 1
  fi
  IFS=":" read -r _cli_service _cli_action <<< "$1"
  if [[ -z "${_cli_service:-}" || -z "${_cli_action:-}" ]]; then
    echo "usage: $0 <service>:<start|stop|status>" >&2
    exit 1
  fi
  run_service_script "$_cli_service" "$_cli_action"
fi
