...
SERVICE_DIR = WORKSPACE_DIR / "gateway"
...
LEGACY_ENV_FILE = SERVICE_DIR / "config.env"
SHARED_ENV_FILE = SERVICE_DIR / "config.shared.env"
SECRET_ENV_FILE = SERVICE_DIR / "config.secret.env"
DOTENV_FILE = SERVICE_DIR / ".env"
ENV_FILE_CANDIDATES = (LEGACY_ENV_FILE, SHARED_ENV_FILE, SECRET_ENV_FILE, DOTENV_FILE)

def _iter_monorepo_config_env_files() -> tuple[Path, ...]:
    config_dir = WORKSPACE_DIR / "config"
    return (
        config_dir / "config.shared.env",
        config_dir / "config.secret.env",
    )
EXPLICIT_ENV_KEYS = (
...
def iter_workspace_env_files() -> tuple[Path, ...]:
    ...
    if config_root is not None:
        service_files = tuple((config_root / filename).resolve() for filename in DEFAULT_ENV_FILENAMES)
        for path in (*ENV_FILE_CANDIDATES, *_iter_monorepo_config_env_files(), *_iter_resource_shared_env_files(), *service_files):
            if path in seen:
                continue
            seen.add(path)
            values.append(path)
