...
def _qa_mode_disabled(request: Request, mode: str) -> bool:
    settings = request.app.state.settings
    return mode not in settings.enabled_qa_modes


def _mode_disabled_json(*, trace_id: str, mode: str, enabled_modes: frozenset[str]) -> JSONResponse:
    return JSONResponse(
        status_code=503,
        content={
            "success": False,
            "code": "QA_MODE_DISABLED",
            "error": "qa_mode_disabled",
            "message": f"QA mode '{mode}' is disabled on this gateway",
            "mode": mode,
...
def _mode_disabled_stream(*, trace_id: str, mode: str, enabled_modes: frozenset[str]) -> StreamingResponse:
    def _frames():
        yield (
            'data: {"type":"error","code":"QA_MODE_DISABLED","error":"qa_mode_disabled",'
            f'"message":"QA mode \'{mode}\' is disabled on this gateway","mode":"{mode}",'
            f'"enabled_modes":{json.dumps(sorted(enabled_modes), ensure_ascii=False)},'
            f'"retriable":false,"trace_id":"{trace_id}"}}\n\n'
        )
...
async def _proxy_ask(request: Request, payload: AskRequest, mode: str) -> JSONResponse:
    ...
    settings = request.app.state.settings
    if _qa_mode_disabled(request, mode):
        return _mode_disabled_json(trace_id=trace_id, mode=mode, enabled_modes=settings.enabled_qa_modes)
    ...
    if _qa_mode_disabled(request, route_decision.actual_mode):
        return _mode_disabled_json(
            trace_id=trace_id,
            mode=route_decision.actual_mode,
            enabled_modes=settings.enabled_qa_modes,
        )
...
async def _proxy_ask_stream(request: Request, payload: AskRequest, mode: str):
    ...
    settings = request.app.state.settings
    if _qa_mode_disabled(request, mode):
        return _mode_disabled_stream(trace_id=trace_id, mode=mode, enabled_modes=settings.enabled_qa_modes)
