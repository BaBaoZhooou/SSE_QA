...
MONOREPO_CONFIG_ENV_FILES="$MONOREPO_CONFIG/config.shared.env:$MONOREPO_CONFIG/config.secret.env"
...
export FASTQA_ENV_FILES="${FASTQA_ENV_FILES:-$MONOREPO_CONFIG_ENV_FILES:$PROJECT_ROOT/config.env:$PROJECT_ROOT/config.shared.env:$PROJECT_ROOT/config.secret.env:$PROJECT_ROOT/.env:$FASTQA_SHARED_ENV_FILES:$FASTQA_SERVICE_CONFIG_ROOT/config.shared.env:$FASTQA_SERVICE_CONFIG_ROOT/config.secret.env:$FASTQA_SERVICE_CONFIG_ROOT/.env:$FASTQA_SERVICE_CONFIG_ROOT/config.env}"
...
if [[ -z "${ENV_FILE_LOADER_PROCESS_KEYS[FASTAPI_PORT]+x}" ]]; then
  export FASTAPI_PORT="${FASTQA_FASTAPI_PORT:-${FASTQA_PORT:-${FASTAPI_PORT:-$APP_PORT}}}"
fi
if [[ -z "${ENV_FILE_LOADER_PROCESS_KEYS[BACKEND_PORT]+x}" ]]; then
  export BACKEND_PORT="${FASTQA_FASTAPI_PORT:-${FASTQA_PORT:-${BACKEND_PORT:-$FASTAPI_PORT}}}"
fi
