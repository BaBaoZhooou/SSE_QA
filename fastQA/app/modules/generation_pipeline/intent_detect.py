...
from app.core.prompt_loader import load_intent_tag_descriptions, load_prompt_template
...
# Descriptions live in prompts/intent_tags.json (externalized).
...
def _intent_tag_descriptions() -> dict[str, str]:
    return load_intent_tag_descriptions()


def _normalize_intent_tag(raw: str) -> str:
    descriptions = _intent_tag_descriptions()
...
def build_intent_detect_system_prompt() -> str:
    ...
    descriptions = _intent_tag_descriptions()
    intent_string = json.dumps(descriptions, ensure_ascii=False)
    keys_line = ", ".join(sorted(descriptions))
    template = load_prompt_template("intent_detect_system.txt")
    return template.format(intent_string=intent_string, keys_line=keys_line)
