...
def _generate_ai_query(
    *,
    ...
    if entity_lock_enabled:
        entity_guardrail_block = load_prompt_template("stage2_entity_guardrail_block.txt")

    core_question = normalized_user_question if normalized_user_question else f"关于{claim_text[:50]}的问题"
    _ = preferred_sections
    _ = filters
    prompt = load_prompt_template("stage2_query_user.txt").format(
        core_question=core_question,
        claim_text=claim_text,
        keywords=', '.join(keywords) if keywords else '无',
        entity_guardrail_block=entity_guardrail_block,
    )
    stage2_query_system = load_prompt_template("stage2_query_system.txt").strip()
            ...
            with chat_lane_pool.lease_lane(trace_label=trace_label) as leased_lane:
                ...
                response = _run_cancelable_upstream_call(
                    call=lambda: active_client.chat.completions.create(
                        model=model,
                        messages=[
                            {"role": "system", "content": stage2_query_system},
