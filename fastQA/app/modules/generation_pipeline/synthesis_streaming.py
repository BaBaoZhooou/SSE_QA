...
STAGE4_FACT_EXTRACTION_PROMPT = load_prompt_template("stage4_fact_extraction.txt")
STAGE4_FACT_SYNTHESIS_PROMPT = load_prompt_template("stage4_fact_synthesis.txt")
STAGE4_RESTRICTED_SYNTHESIS_PROMPT = load_prompt_template("stage4_restricted_synthesis.txt")
STAGE4_STRUCTURE_ONLY_PROMPT = load_prompt_template("stage4_structure_only.txt")
STAGE4_SYSTEM_PROMPT_TEMPLATE = load_prompt_template("stage4_system.txt")
...
def iter_stage4_synthesis_with_pdf_chunks(
    *,
    ...
    try:
        ...
        stage4_effective_min_citations = (
            min(stage4_min_citations, len(top5_with_scores))
            if top5_with_scores
            else stage4_min_citations
        )
        logger.info(
            "stage4 reference policy topk=%s min_citations=%s effective_min_citations=%s element_guard=%s citation_verify=%s two_stage=%s structure_only=%s top_ref_count=%s top_ref_sample=%s",
            stage4_topk,
        ...
        _cap_top = min(stage4_topk, len(top5_with_scores)) if top5_with_scores else stage4_topk
        citation_requirement = (
            f"必须至少引用{stage4_effective_min_citations}篇不同的文献（最多{_cap_top}篇）"
        )
        detail_requirement = (
            "每个核心要点必须包含机理解释与定量信息，并点明该信息与用户问题的关联；"
            "带 `(doi=…)` 的句子不能只堆数字，须在同句或紧邻一句内完成「证据结论 + 为何重要/如何作用」的简要专业解读；"
            "**凡含具体数值、区间、单位（含 g/cm³、%、μm、mAh/g 等）的 `(doi=…)` 句子，数字必须与对应证据片段一致，禁止改写、合并区间或补全文献未给出的数字**"
        )
        fact_mode_rules = ""
...
- **禁止「同一 DOI 背靠背重复」**：不要写成「…`(doi=A)`。」「`(doi=A)` 下一句话…」；同一篇顺延论述合并为一段，**段末一处** `(doi=A)` 即可。**不得**在列表有多篇可用时，把本应属于其他 doi 的要点也全部写成同一篇。"""
            fact_mode_rules = f"""
...
{diversity_block}"""
        ...
        system_prompt = STAGE4_SYSTEM_PROMPT_TEMPLATE.format(
            citation_requirement=citation_requirement,
            detail_requirement=detail_requirement,
            fact_mode_rules=fact_mode_rules,
        )
        ...
        if (
            ...
            and len(cited_dois) < stage4_effective_min_citations
